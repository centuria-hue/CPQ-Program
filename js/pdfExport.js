...
window.generateQuotePDF = function generateQuotePDF(configState) {
  const { salesName, customerName, projectName, model, selectedOptions, customColorValue, customFirmwareValue, customOtherValue, calculations, quoteDocId } = configState;
  
  // Format dates
  const today = new Date();
  const dateStr = today.toISOString().split('T')[0];
  const quoteId = quoteDocId || `TR-${Math.floor(100000 + Math.random() * 900000)}`;
  ...
  if (model.customizableSpecs) {
    Object.keys(model.customizableSpecs).forEach(key => {
      ...
      if (opt) {
        ...
        let nreDisplay = opt.nreFee > 0 ? `$${opt.nreFee.toLocaleString()}` : '$0';
        ...
        } else if (key === 'otherCustomization' && selectedId === 'other_custom') {
          const othText = customOtherValue ? customOtherValue.trim().replace(/\n/g, '<br/>') : 'Client Specified Requirements';
          finalSpecValue = `Non-Standard Customization:<br/><div style="margin-top:4px; font-family:monospace; background:#f1f5f9; padding:6px; border-radius:4px; border:1px solid #cbd5e1; font-size:11px; white-space:pre-wrap; page-break-inside:avoid; break-inside:avoid;">${othText}</div>`;
          nreDisplay = '<span style="color:#d97706; font-size:11px;">TBD (Quoted after R&D evaluation)</span>';
        }
  ...
  if (modifiedOnly.length > 0) {
    customSummaryHtml = `
      <div style="margin-bottom: 20px; border: 2px dashed #0284c7; background: #f0f9ff; border-radius: 6px; padding: 14px; page-break-inside: avoid; break-inside: avoid;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 12px; background: #ffffff; border: 1px solid #bae6fd; line-height: 1.5;">
          <thead>
            <tr style="background: #e0f2fe; color: #0369a1; text-align: left; page-break-inside: avoid; break-inside: avoid;">
              ...
              <th style="padding: 8px; border-bottom: 1px solid #bae6fd; width: 12.5%;">Addon Delta (USD)</th>
              <th style="padding: 8px; border-bottom: 1px solid #bae6fd; width: 12.5%;">NRE Fee (USD)</th>
            </tr>
          </thead>
          <tbody>
            ${modifiedOnly.map(item => `
              <tr style="border-bottom: 1px solid #e2e8f0; page-break-inside: avoid; break-inside: avoid;">
                <td style="padding: 8px; font-weight: bold; color: #0f172a; vertical-align: top; line-height: 1.5; word-break: break-word;">${item.targetKey}</td>
                <td style="padding: 8px; font-family: monospace; font-weight: bold; color: #0284c7; vertical-align: top; line-height: 1.5;">${item.pnCode}</td>
                <td style="padding: 8px; color: #0f172a; vertical-align: top; line-height: 1.5; word-break: break-word;">
                  ${item.specValue}
                </td>
                <td style="padding: 8px; color: #059669; font-weight: bold; vertical-align: top; line-height: 1.5;">
                  ${item.addonPrice > 0 ? `+$${item.addonPrice.toLocaleString()}` : '$0'}
                </td>
                <td style="padding: 8px; color: #d97706; font-weight: bold; vertical-align: top; line-height: 1.5;">
                  ${item.nreDisplay}
                </td>
              </tr>
            `).join('')}
  ...
  model.specifications.forEach(cat => {
    specsHtml += `
      <div class="pdf-spec-block" style="margin-top: 16px; page-break-inside: avoid; break-inside: avoid;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 4px; font-size: 12px; line-height: 1.5;">
    `;
    ...
    cat.items.forEach(item => {
      ...
      if (item.customizable && item.customKey && selectedOptions[item.customKey]) {
        ...
        if (matchedOpt) {
          ...
          if (item.customKey === 'casingColor' && optionId === 'custom_color') {
            ...
            const fwText = customFirmwareValue ? customFirmwareValue.trim().replace(/\n/g, '<br/>') : 'Client Specified Requirements';
            displayValue = `Customized FW Build:<br/><div style="margin-top:4px; font-family:monospace; background:#f8fafc; padding:6px; border-radius:4px; border:1px solid #e2e8f0; font-size:11px; white-space:pre-wrap; page-break-inside:avoid; break-inside:avoid; line-height:1.4;">${fwText}</div><div style="font-size:10px; color:#d97706; margin-top:2px; line-height:1.3;">*NRE fee will be quoted separately after R&D engineering workload evaluation.</div>`;
          } else if (item.customKey === 'otherCustomization' && optionId === 'other_custom') {
            const othText = customOtherValue ? customOtherValue.trim().replace(/\n/g, '<br/>') : 'Client Specified Requirements';
            displayValue = `Non-Standard Customization:<br/><div style="margin-top:4px; font-family:monospace; background:#f8fafc; padding:6px; border-radius:4px; border:1px solid #e2e8f0; font-size:11px; white-space:pre-wrap; page-break-inside:avoid; break-inside:avoid; line-height:1.4;">${othText}</div><div style="font-size:10px; color:#d97706; margin-top:2px; line-height:1.3;">*NRE fee & MOQ will be quoted separately after R&D engineering workload evaluation.</div>`;
          }
      ...
      specsHtml += `
        <tr style="border-bottom: 1px solid #e2e8f0; ${isCustomized ? 'background: #f0f9ff;' : ''} page-break-inside: avoid; break-inside: avoid;">
          <td style="padding: 7px 10px; font-weight: bold; width: 35%; color: #475569; vertical-align: top; line-height: 1.5; word-break: break-word;">
            ${item.key} ${isCustomized ? '<span style="color:#0284c7; font-size:10px; font-weight:bold;">[CUSTOMIZED]</span>' : ''}
          </td>
          <td style="padding: 7px 10px; width: 65%; color: ${isCustomized ? '#0369a1' : '#0f172a'}; ${isCustomized ? 'font-weight:bold;' : ''} vertical-align: top; line-height: 1.5; word-break: break-word;">
            ${displayValue}
          </td>
        </tr>
      `;
  ...
  element.innerHTML = `
    ...
    <div style="border-bottom: 3px solid #0284c7; padding-bottom: 14px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: flex-end; page-break-inside: avoid; break-inside: avoid;">
      <div>
        <h1 style="margin: 0; font-size: 22px; color: #0f172a; font-weight: 800; line-height: 1.3;">VIVOTEK <span style="font-size:13px; color:#64748b; font-weight: normal;">A Delta Group Company</span></h1>
        <h2 style="margin: 3px 0 0 0; font-size: 15px; color: #0284c7; font-weight: 700; line-height: 1.3;">Transportation Vertical Solution - Specification & Customization Sheet</h2>
      </div>
    ...
    <!-- Info Grid -->
    <table style="width: 100%; border: 1px solid #cbd5e1; border-collapse: collapse; margin-bottom: 16px; font-size: 12px; background: #f8fafc; page-break-inside: avoid; break-inside: avoid; line-height: 1.4;">
      <tr>
        <td style="padding: 8px 12px; border-right: 1px solid #cbd5e1; width: 50%; vertical-align: top;">
    ...
    <div style="border: 2px solid #0284c7; border-radius: 6px; padding: 14px; margin-bottom: 18px; background: #ffffff; page-break-inside: avoid; break-inside: avoid; line-height: 1.5;">
      <div style="display: flex; justify-content: space-between; align-items: flex-start;">
        <div>
          <h3 style="margin: 0 0 4px 0; color: #0369a1; font-size: 17px; line-height: 1.3;">${model.displayName}</h3>
          <p style="margin: 0 0 10px 0; font-size: 12px; color: #334155; line-height: 1.4;">${model.description}</p>
        </div>
      </div>

      <!-- Generated Primary Part Number -->
      <div style="background: #f0f9ff; border: 1px solid #bae6fd; padding: 8px 12px; border-radius: 4px; margin-bottom: 10px; font-family: monospace; font-size: 13px; color: #0369a1; font-weight: bold; line-height: 1.4;">
      ...
      <table style="width: 100%; font-size: 12px; border-top: 1px solid #e2e8f0; padding-top: 8px; line-height: 1.4;">
        <tr>
          <td style="padding: 4px 0; width: 33%;"><strong>Custom Addon Delta (USD):</strong> <span style="color:#059669; font-weight:bold;">${calculations.totalAddon >= 0 ? '+' : ''}$${calculations.totalAddon.toLocaleString()}</span></td>
          <td style="padding: 4px 0; width: 33%;"><strong>NRE Setup Fee (USD):</strong> <span style="color:#d97706; font-weight:bold;">$${calculations.totalNRE.toLocaleString()}</span></td>
    ...
    <!-- Terms -->
    <div style="margin-top: 24px; border-top: 1px solid #cbd5e1; padding-top: 12px; font-size: 10.5px; color: #64748b; line-height: 1.5; page-break-inside: avoid; break-inside: avoid;">
  ...
  if (window.html2pdf) {
    const opt = {
      margin:       [10, 10, 10, 10],
      filename:     `VIVOTEK_CPQ_${model.id}_${quoteId}_${dateStr}.pdf`,
